...
ggplot(data=msleep, mapping=aes(x=vore,y=bodywt))+
  ...
  labs(title="Body weight by vore type",x="Vore type",y="Body weight")+
  scale_y_log10()
```
...
ggplot(data=msleep, mapping=aes(x=bodywt,y=sleep_total))+
  ...
  scale_x_log10()
```
There is a negative relationship between body weight and sleep time (as body weight inc, sleep time dec)
...
ggplot(data=msleep, mapping=aes(x=vore,y=sleep_total))+
  geom_boxplot(mapping=aes(fill=vore))+
  labs(title="Sleep total by vore type",x="Vore type",y="Sleep total")
```
The insecti vore tends to sleep the most. 
...
```{r}
ggplot(data=msleep, mapping=aes(x=brainwt,y=sleep_total))+
  geom_point()+
  geom_smooth(method=lm)+
  labs(title="Sleep total by brain weight",x="Brain weight",y="Sleep total")+
    coord_cartesian(xlim=c(0, .75), ylim=c(0, 24))
