...
```{r message=FALSE, warning=FALSE}
spider_records <- read.csv("data/spider_records.csv") %>% clean_names()
spiders_locations <- read.csv("data/spiders_locations.csv") %>% clean_names()
```
...
```{r}
spiders <- full_join(spider_records, spiders_locations, by= "accession")
```
...
```{r}
write.csv(spiders, file="spiders_with_locs.csv", row.names = F)
```
