...
Height_ft <- c(29029, 28251, 28169, 27940, 27838)
...
```{r}
mountains <- tibble("Name" = Name,"Feet" = Height_ft,"Meters" = Height_m)
mountains
```
...
mountains <- add_column(mountains, "First climbed" = first_climbed)
```
...
mountains <- add_column(mountains, "Summits" = summits)
```
...
mountains <- add_column(mountains, "Fatalities" = fatalities)
```
